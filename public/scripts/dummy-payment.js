...
async function processPayment() {
  ...
  if (!orderType || !orderIdInput || orderType === "unknown" || orderIdInput === "unknown") {
  ...
  // Generate orderId and timestamp
  const orderId = `ORD-${Date.now()}`;
  
  // Create accessory object (copy of orderAccessory to preserve all fields)
  const accessory = { ...orderAccessory };
  ...
  const orderData = {
    ...
    items: [{
      type: orderType,
      id: orderIdInput,
      accessory: accessory, // Use the full accessory object
      quantity: 1,
      amount: amount
    }],
    totalAmount: amount,
